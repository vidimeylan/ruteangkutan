...
<head>
	...
	$(function() {
        ...
        jQuery("#crud").jqGrid({
			...
			autowidth: true,
			multiselect:true,
		   	url: '<?php echo base_url().index_page();?>/user/read',
		   	mtype:'POST',
			datatype: "json",
		   	colNames:['Id','Username','Password', 'Name'],
		   	colModel:[
		   		{name:'USER_ID',index:'USER_ID', width:55, editable:false, editoptions:{readonly:true}, sorttype:'int', hidden : true},
		   		{name:'USER_NAME',index:'USER_NAME', width:100,editable:true},
		   		{name:'PASSWORD',index:'PASSWORD', width:100,editable:true ,hidden: false , edittype:'password'},
		    ...
		    viewrecords: true,
		    sortorder: "asc",
		    editurl: '<?php echo base_url().index_page();?>/user/action',
		    caption:"User"
		});
